...
* Run this file before replace_job_title_mysql.py

! Uncomment "SQL ONLY IMPLEMENTATION" sections for SQL only implementation
*/

DROP TABLE IF EXISTS la_payroll_data_copy;
CREATE TABLE la_payroll_data_copy
LIKE la_payroll_data;

INSERT la_payroll_data_copy
SELECT *
...
SET EMPLOYMENT_TYPE = 
	CONCAT(
    CONCAT(LEFT(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', 1), 1), RIGHT(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', 1), CHAR_LENGTH(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', 1)) - 1)),
		' ',
		CONCAT(LEFT(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', -1), 1), RIGHT(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', -1), CHAR_LENGTH(SUBSTRING_INDEX(EMPLOYMENT_TYPE, '_', -1)) - 1))
  );
...
SET JOB_STATUS = 
	CONCAT(
    CONCAT(LEFT(SUBSTRING_INDEX(JOB_STATUS, '_', 1), 1), RIGHT(SUBSTRING_INDEX(JOB_STATUS, '_', 1), CHAR_LENGTH(SUBSTRING_INDEX(JOB_STATUS, '_', 1)) - 1)),
		' ',
		CONCAT(LEFT(SUBSTRING_INDEX(JOB_STATUS, '_', -1), 1), RIGHT(SUBSTRING_INDEX(JOB_STATUS, '_', -1), CHAR_LENGTH(SUBSTRING_INDEX(JOB_STATUS, '_', -1)) - 1))
  )
WHERE JOB_STATUS REGEXP '_';
...
UPDATE la_payroll_data_copy
SET ETHNICITY = 'N/A'
WHERE ETHNICITY LIKE '' OR ETHNICITY LIKE 'NA' OR ETHNICITY LIKE 'NOT APPLICABLE' OR ETHNICITY LIKE 'UNKNOWN';
...
WHERE JOB_CLASS_PGRADE LIKE '' OR JOB_CLASS_PGRADE LIKE 'NA';
...
UPDATE la_payroll_data_copy
SET MOU_TITLE = 'N/A'
WHERE MOU_TITLE LIKE '' OR MOU_TITLE LIKE 'NA';
...
JOIN mou_replacement r
ON p.MOU_TITLE = r.mou_title AND p.MOU = r.old_mou
SET p.MOU = r.new_mou;
...
UPDATE la_payroll_data_copy p
JOIN ladwp_mou_title_replacement r
ON p.MOU_TITLE = r.old_mou_title AND p.MOU = r.old_mou
SET p.MOU_TITLE = r.new_mou_title,
  p.MOU = r.new_mou;
...
UPDATE la_payroll_data_copy
SET JOB_TITLE = 'N/A'
WHERE JOB_TITLE LIKE '' OR JOB_TITLE LIKE 'NA';
...
UPDATE la_payroll_data_copy p
JOIN temp_job_replacement r
ON p.JOB_TITLE REGEXP CONCAT('(^|\\s)', r.abbreviation, '(\\s|$)')
SET p.JOB_TITLE = TRIM(REGEXP_REPLACE(
  p.JOB_TITLE,
  CONCAT('(^|\\s)', r.abbreviation, '(\\s|$)'),
  CONCAT(' ', r.full_word, ' ')
));
*/
...
CREATE TABLE job_title_word_replacement (
  ...
  replacement_type ENUM('ABBREVIATION', 'REPLACEMENT', 'SPELLING') NOT NULL,
...
/* SQL ONLY IMPLEMENTATION:
UPDATE la_payroll_data_copy p
JOIN job_title_word_replacement r
ON p.JOB_TITLE LIKE r.match_condition
SET p.JOB_TITLE = TRIM(REGEXP_REPLACE(
  p.JOB_TITLE,
  CONCAT('(^|\\s)', r.incorrect_word, '(\\s|$)'),
  CONCAT(' ', r.correct_word, ' ')
));
*/
UPDATE la_payroll_data_copy
